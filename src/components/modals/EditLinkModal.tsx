...
import { Edit, Link as LinkIcon, Tag, X, Loader2, FileText } from 'lucide-react';
...
import { getPlatformIcon, fetchWebsiteMetadata } from '@/lib/platform-utils';
...
export default function EditLinkModal({
  ...
  useEffect(() => {
    if (isOpen && link) {
      ...
      setTitleFetched(true); // Already has title
      setShowDescription(!!link.description); // Show description if it exists
  ...
  // Enhanced metadata fetching for edits (if URL changes)
  const fetchMetadata = useCallback(async (url: string) => {
    ...
    try {
      ...
      if (platform === 'youtube') {
        ...
        // For websites, try to fetch metadata
        try {
          ...
          // Silent fail for metadata fetching
        }
      }
      ...
      if (title && title.length > 0) {
        ...
        addToast({
          ...
          description: platform === 'youtube' ? 'Video title has been automatically fetched.' : 'Website title has been automatically fetched.',
          type: 'success'
        });
      }
    } catch {
  ...
  // Debounced URL processing for metadata fetching
  useEffect(() => {
    if (!formData.url) {
  ...
  }, [formData.url, fetchMetadata, titleFetched]);
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[95vh] overflow-y-auto mobile-modal edit-link-modal p-0">
        ...
        <div className="sm:px-6 px-4">
          ...
          <div className="sm:space-y-2 space-y-1.5">
            ...
            <Input
               id="title"
               placeholder={titleFetched ? "Title fetched automatically" : "Enter title (auto-fetch for supported sites)"}
