...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const addToast = React.useCallback((toast: Omit<ToastProps, 'id'>) => {
    ...
    const newToast: ToastProps = {
      ...toast,
      id,
      duration: toast.duration ?? 2000, // Reduced from 5000ms to 2000ms for quicker disappearance
    };
    ...
    if (newToast.duration && newToast.duration > 0 && !newToast.action) {
      ...
      // For toasts with actions, auto-remove after 4 seconds (reduced from 8)
      setTimeout(() => {
        dispatch({ type: 'REMOVE_TOAST', id });
      }, 4000);
    }
...
function Toast({ title, description, type = 'info', action, onClose }: ToastProps) {
  ...
  return (
    <div
      className={cn(
        'relative flex items-start gap-2 p-3 border rounded-md shadow-md animate-in slide-in-from-right-full duration-200',
        colors[type]
      )}
    >
      ...
      <div className="flex-1 min-w-0">
        {title && (
          <div className="font-medium text-xs mb-0.5">{title}</div>
        )}
        {description && (
