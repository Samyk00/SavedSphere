...
import { Search, X, Filter, Hash, Calendar, Globe } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import PlatformFilterButton from '@/components/ui/PlatformFilterButton';
...
export default function SearchBar({
  ...
  return (
    <TooltipProvider>
      <div className="relative bg-white px-6 py-3 sm:py-4 border-b border-gray-200">
                ...
                <TooltipTrigger asChild>
                  <div>
                    <PlatformFilterButton
                      ...
                      count={availablePlatforms.find(p => p.id === 'youtube')?.count}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Filter by YouTube videos</p>
                </TooltipContent>
              </Tooltip>
                ...
                <TooltipTrigger asChild>
                  <div>
                    <PlatformFilterButton
                      ...
                      count={availablePlatforms.find(p => p.id === 'instagram')?.count}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Filter by Instagram posts</p>
                </TooltipContent>
              </Tooltip>
                ...
                <TooltipTrigger asChild>
                  <div>
                    <PlatformFilterButton
                      ...
                      count={availablePlatforms.find(p => p.id === 'facebook')?.count}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Filter by Facebook posts</p>
                </TooltipContent>
              </Tooltip>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div>
                    <PlatformFilterButton
                      platform="linkedin"
                      isSelected={selectedPlatforms.includes('linkedin')}
                      onClick={() => onPlatformQuickFilter?.('linkedin')}
                      count={availablePlatforms.find(p => p.id === 'linkedin')?.count}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Filter by LinkedIn posts</p>
                </TooltipContent>
              </Tooltip>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div>
                    <PlatformFilterButton
                      platform="reddit"
                      isSelected={selectedPlatforms.includes('reddit')}
                      onClick={() => onPlatformQuickFilter?.('reddit')}
                      count={availablePlatforms.find(p => p.id === 'reddit')?.count}
                    />
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Filter by Reddit posts</p>
                </TooltipContent>
              </Tooltip>
          ...
          <div className="hidden sm:block">
            <div className="relative w-64">
              ...
              <Input
                ...
                className="pl-10 pr-10 h-8 sm:h-10 text-sm rounded-lg border border-gray-200 hover:border-gray-300 transition-all duration-200 bg-gray-50"
              />
              {searchQuery && (
                <Button
                  variant="ghost"
        ...
        <DropdownMenu>
          <Button
            variant="ghost"
            size="sm"
            className="relative h-8 w-8 sm:h-10 sm:w-10 p-0 rounded-lg transition-all duration-200 hover:shadow-depth-1"
        ...
        <div className={`${isMobile ? 'hidden' : 'absolute top-full left-6 mt-3 z-10'} ${searchQuery && !title?.includes('All Links') && !title?.includes('Favorites') && !title?.includes('Trash') && !title?.includes('Folder') ? 'sm:hidden' : ''}`}>
        ...
        <div className={`${isMobile ? 'hidden' : 'absolute top-full right-4 mt-2 z-10'}`}>
