...
export default function AppLayout() {
  ...
  // Handle mobile sidebar collapse - only collapse on very small screens
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth < 640) {
        setSidebarCollapsed(true);
      } else if (window.innerWidth >= 640) {
        // Only auto-expand if it was collapsed due to small screen
  ...
  const handleSaveLink = async (linkData: LinkFormData) => {
    try {
      ...
      // Handle error with user notification if needed
    }
  };
  ...
  return (
    <div className="h-screen flex bg-white text-gray-900">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Main Content Area */}
        <main className="flex-1 overflow-y-auto pt-11 bg-white">
          {/* Bulk Actions */}
          <BulkActions
            folders={folders}
      ...
      <FloatingActionButton onClick={handleAddLinkClick} />
